...
interface Props {
  ...
  description?: string; // เพิ่ม prop ใหม่
}

const AccordionInfo: FC<Props> = ({
  panelClassName = "p-4 pt-3 last:pb-0 text-slate-600 text-sm dark:text-slate-300 leading-6",
  data = DEMO_DATA,
  description, // รับ prop นี้
}) => {
  // สร้างข้อมูลใหม่ที่รวม description
  const accordionData = [
    { name: "รายละเอียดสินค้า", content: description || "" }, // เพิ่มข้อมูลนี้เข้าไป
    ...data, // ข้อมูลเดิม
  ];
